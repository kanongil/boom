...
exports.Boom = class Boom extends Error {
    ...
    constructor(message, options = {}) {
        ...
        const { statusCode = 500, data, headers, decorate, ctor = exports.Boom } = options;
        const causeOption = 'cause' in options ? { cause: options.cause } : undefined;

        super(message ?? internals.codes.get(statusCode) ?? 'Unknown', causeOption);
        ...
        if (causeOption) {
            this.cause ??= causeOption.cause;                          // Explicitly assign cause to work with old runtimes
        }
...
exports.boomify = function (err, options = {}) {
    ...
    if (err.hasOwnProperty('isServer')) {
...
internals.statusError = function (statusCode, massage) {
    ...
    const method = massage ?
        function (...args) {
            ...
            const [message, options] = massage(...args);
            return new exports.Boom(message, { statusCode, ctor: method, ...options });
        } :
...
exports.badImplementation = internals.statusError(500, (message, data) => {
    ...
    const res = internals.serverError(message, data);
    res[1].decorate = { isDeveloperError: true };
    return res;
});
